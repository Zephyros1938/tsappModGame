...
def _check_int_range(value, bits, signed=True):
    ...
    if signed:
        if not (-(2**(bits - 1)) <= value < 2**(bits - 1)):
            raise OverflowError(f"Value {value} out of range for {bits}-bit signed integer")
    else:
        if not (0 <= value < 2**bits):
            raise OverflowError(f"Value {value} out of range for {bits}-bit unsigned integer")
    return value
...
class DateTime(datetime.datetime):
    ...
    def __sub__(self, other):
        if isinstance(other, TimeSpan):
            ...
            return TimeSpan(days=result.days, seconds=result.seconds, microseconds=result.microseconds)
        return NotImplemented
